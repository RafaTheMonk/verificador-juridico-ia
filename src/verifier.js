/**
 * Orquestrador principal do verificador.
 *
 * Fluxo:
 *   L0 parse -> decide fonte -> L1 existência -> L2 conteúdo -> L3 adequação -> recomendação
 */
...
export async function verificar({ referencia, contexto }) {
  ...
  // Rejeição imediata se DV inválido
  ...
  if (dvInvalido) {
    ...
    try {
      ...
      if (r.encontrado) {
        ...
        const ufReal = (() => {
          // Procura padrão /UF no bloco raw do primeiro resultado
    ...
    existencia.flags.push(`Tipo ${parse.tipo} / tribunal ${parse.tribunalInferido || "?"} sem cobertura na Camada 1 (fase Básica).`);
  }

  // Inclui flags locais no bloco de existência para visibilidade
  ...
  const resposta = {
    ...
    conteudo: {
      ...
      tema_repetitivo: null, // fase básica não cobre
